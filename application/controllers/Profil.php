...
class Profil extends CI_Controller
{
	...
	public function keranjang($id)
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Data Baju Berhasil Ditambahkan ke keranjang!</div>');
	...
	public function pesanan()
	{
		...
		for ($i = 0; $i < $jumlah_beli; $i++) {
			array_push($data_detail, ['id_baju' => $this->input->post('baju')[$i]]);
		...
		if ($this->Penjualan_model->insert($data_p, $upload_image) && $this->Detail_model->insert($data_detail)) {
			for ($i = 0; $i < $jumlah_beli; $i++) {
				$this->Baju_model->min_stok($data_detail[$i]['jumlah'], $data_detail[$i]['id_baju']) or die('gagal min stok');
			}
